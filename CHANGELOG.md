...
- 优化脚本默认值、重复安装/重复查询和无 TTY 执行兼容性
- 修复附加 Windows Dockerfile 换行构建错误与部分等待循环边界问题
...
- 添加从路由器层面获取IPv6子网掩码的功能，使用rdisc6工具检测真实的IPv6前缀长度
- 当检测到的路由器通告前缀与接口配置前缀不一致时，会自动使用更大（更准确）的前缀
- 自动安装ndisc6软件包以支持IPv6路由器发现功能

2025.08.24

- 添加支持在Docker环境安装的时候，选择是否安装为可进行硬盘大小限制，选择是否非默认路径安装，选择安装的存储池大小
- 检测btrfs存储时，开设容器添加可支持设置硬盘大小的选项
- 添加脚本可自行检测Docker是否真正限制了容器硬盘的大小
