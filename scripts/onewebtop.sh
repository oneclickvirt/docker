...
if docker inspect "webtop_${http_port}" >/dev/null 2>&1; then
  _red "Container webtop_${http_port} already exists, please remove it before creating a new one."
  _red "容器 webtop_${http_port} 已存在，请先删除后再重新创建。"
  exit 1
fi
